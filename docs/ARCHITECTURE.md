...
src/
  ...
  project.rs     — `.dpaint` v1、タイル重複排除、CRC、原子的保存
  tools/
...
- メモリ会計は診断用に維持するが、op は件数/バイト上限で論理削除しない。`max_steps` 互換設定は履歴パネルの表示件数として扱う。
...
- 各操作は約44×40pxの縦タイルにし、上段へPainterアイコン、下段へ短い静的日本語ラベルを描く。一体のクリック領域・hover/selected背景・`WidgetInfo`(名前/有効/選択状態)を持つ。最小幅では4行以内を目安に `pack_rows` し、行端の孤立separatorを省略する。
...
## 18.2 設定(Preferences)ダイアログと履歴表示数(SPEC §34)
...
- OK 確定時: 互換キー `history.max_steps` を更新して即保存し、全タブの履歴パネル表示上限へ反映する。履歴エントリは切り詰めない。
- Ctrl+K を keymap.rs に追加(`Action::OpenPreferences`)。ツールバーにも歯車アイコンのボタンを 1 つ(§33 末尾)。

## 18.3 History のラベル付け・全件保持・ジャンプ
...
pub struct History {
    ...
    max_steps: usize,     // 履歴パネル表示上限。既定 50、Settings から注入
    // ...
}
```
...
- **全件保持**: 新規pushはredo枝だけを通常どおり破棄し、undoの古いentryは件数/バイトで削除しない。`set_max_steps` は互換APIとして表示上限だけを更新する。
- **ジャンプ**: `History::jump_to(&mut doc, target_len: usize)` は既存の単発 `undo()`/`redo()` を `target_len` に達するまでループ呼び出しする薄い関数として実装する。表示中の行から呼ぶ通常経路では最大でも表示件数程度の移動になる。
- 履歴パネルは現在位置前後の設定件数だけを描画し、省略時は古い履歴もプロジェクトへ保存される旨を表示する。これにより全件保持でも毎フレームのウィジェット数を制限する。
...
- 表示は `egui::ScrollArea::vertical()` 内に、現在位置の前後から設定件数分を切り出す。古い undo 行を省略した場合は「古い履歴もプロジェクトに保存されます」と注記し、初期状態が表示範囲内なら仮想「(初期状態)」行を描く。undo は古い順、redo は直近の undo ほど上に淡色で表示し、現在位置をハイライトする。
...
### V6-M1 — History のラベル付け・全件保持・ジャンプ(UIなし)
§18.3 全部。全 push 呼び出し箇所にラベルを追加、全件保持と `jump_to` を実装。表示件数を変更しても undo/redo エントリが削除されないことをテストで担保。
...
# v7 設計 (`.dpaint` v1・ラベル付き操作タイル)

## 19.1 project.rs

- `History::project_entries()` はundoを古い順、redoを内部スタック順で借用公開する。保存時は `undo.iter().chain(redo.iter().rev())` を線形revision順とし、`cursor = undo.len()` を記録する。読み込み時はcursorで分割し、後半をreverseしてredo内部順を戻す。
- `HistoryOp` 7種を固定u8 tagで明示的に符号化する。Rustのメモリ表現やenum判別値をそのまま書かない。Layer/DocSnapshot/PatchRegionも全フィールドをLE長付き形式で書く。
- 現在Documentはcloneせず借用したままタイル化する。各Layerは256×256格子のtile id列となる。Patchのbefore/afterや履歴内Layer/Snapshotも同じ`TileStore`を通し、FNV-1a 64候補+bytes一致で重複排除する。
- headerは`DPAINT\x1a\0`、version=1、little endian、header size、flags。`META` / `TILS` / `DOCS` / `REVS` chunkを持ち、各chunkにu64長+CRC32(IEEE)を付ける。未知chunkもCRCを検証してskipする。
- decodeは8192px、64layer、100万revision、400万tile、文字列1MiB、encoded+復元バッファ総量2GiB等の防御上限とchecked算術を通す。tile/revision/patch/layerの件数は残りbyte数から求めた最小符号化長と照合してから、`try_reserve_exact`で確保する。ファイル本体も64KiBずつ上限付きで読むため、metadata検査後のfile growthを無制限な`read_to_end`へ渡さない。
- decode後はDOCSのカーソル状態からundo側を逆算し、全revisionを時系列順に状態機械検証する。Patch/Add/Duplicate/Remove/Move/Mergeの添字、各操作時の寸法・レイヤー数、ReplaceAll前後、カーソル時点のDOCS構造が一致しない入力をHistoryへ渡さない。
- 保存は同一dirのcreate_new tmpへ書いてflush/sync_allし、標準ライブラリの単一renameで新規作成/既存置換する。旧pathを先に移動しないためユーザー空間の欠落窓を作らず、失敗時はtmpを清掃する。各chunk追加前のchecked累積長と完成長に加え、loaderと共通分類の復元メモリ予算を保存前にも強制する。decode完了時は動的会計とwriter側の事前計算値が一致することも検査し、両者のdriftを防ぐ。
...
## 22.6d P4.5 — メニューバーの整理(SPEC §59)

- `ui/menu.rs` の項目テーブルを **2 階層**にする: `MenuSlot::Item(MenuAction)` / `MenuSlot::Group(MenuGroup)`(`MenuGroup { label, icon, items: &[MenuAction] }`)。トップレベルの並びは SPEC §59 の順序を単一の `const` テーブルで持つ(表記・順序の二重管理をしない)。
- グループのポップアップは `egui::Popup`/`Area`(既存の「最近使ったファイル」ポップアップと同じ機構)。行は `アイコン + 完全名 + keymap::label_for(...)` の右寄せショートカット。有効判定は既存の `MenuState` 由来の関数をそのまま使い、**グループタイルの有効判定は「中の項目のいずれかが有効」**。
- 折り返し計算(`pack_rows`)はスロット数がタイル数と一致するので既存ロジックのまま。**1280px で 1 行・640px で 2 行以内**をテストで固定する(現行の行数テストを置き換える)。
- 「最近使ったファイル」はグループ内のさらにネストではなく、ファイル…ポップアップ内の**区切り線付き一覧セクション**として展開する(2 段ポップアップを作らない — 操作が増えるため)。
- ショートカット・`keymap.rs`・各アクションの実処理は無変更(配置だけの変更)。アクセシビリティ名(`Response::widget_info` 等)はグループタイルにも付ける。
